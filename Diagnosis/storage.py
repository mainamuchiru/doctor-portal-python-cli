...
import os
class StorageEngine:
    def __init__(self, kb_path="knowledge_base.json", diagnosis_path="session.json"):
        self.kb_path =kb_path# the kb is a variable representing knowledge_base
        self.diagnosis_path= diagnosis_path

    @classmethod
    def from_default_config(cls)->"StorageEngine":
        """ Factory Method: Instantiates StorageEngine with exact project files."""
        return cls (kb_path="knowledge_base.json", diagnosis_path="session.json")
    ...
    def search_diseases_by_symptom(self, symptom_query):
        ...
        kb_data=self.load_diagnosis_History(self)
